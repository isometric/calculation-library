...
## [0.51.4](https://github.com/isometric/calculation-library/releases/tag/v0.51.4)

Dependency version update.

## [0.51.3](https://github.com/isometric/calculation-library/releases/tag/v0.51.3)

Dependency version update.

## [0.51.2](https://github.com/isometric/calculation-library/releases/tag/v0.51.2)

Dependency version update.

## [0.51.1](https://github.com/isometric/calculation-library/releases/tag/v0.51.1)
...
## [0.49.1](https://github.com/isometric/calculation-library/releases/tag/v0.49.1)

Dependency version update.

## [0.49.0](https://github.com/isometric/calculation-library/releases/tag/v0.49.0)

### Added

- `biosphere.allometric_equations.westfall_nsvb`: the above-ground biomass equations of the USDA Forest Service National Scale Volume and Biomass framework (Westfall et al. 2024, GTR-WO-104), which is the allometry the FIA program now applies to every measured tree in the United States. Covers total wood and bark (table S8) and foliage (table S9), and their sum, total AGB; volume, roots and the woodland junipers and pinyons are out of scope. The interface is metric (`dbh_cm`, `height_m`, tonnes) while the equations are imperial, so the conversion happens inside the model. `README.md` in the package records the scope and the provenance of the coefficient tables
- `biosphere.allometric_equations.westfall_nsvb.species`: `get_species` and `find_species_code` resolve the FIA species reference (`SPCD`, Jenkins group, wood specific gravity) by code or by binomial, which is where a tree reaching the allometry starts. `find_species_code` takes the binomial as one string spelled the way `wood_density.get_wood_density` spells it, so a tree type carried by activity data reaches either lookup through the existing `wood_density.tree_type_to_species`, and `NsvbSpecies.species` is that same binomial while `specific_epithet` holds the epithet FIA stores. A bare genus resolves to FIA's genus-level code, the rank a `taxonomic_rank:genus:` qualifier carries. Ten binomials in the reference carry more than one code, and those raise rather than resolving to whichever came first, because the codes are not interchangeable - *Pinus elliottii* is both slash pine and Honduras pine, 26% apart in wood specific gravity. Woodland junipers and pinyons raise too rather than silently mis-predicting: NSVB models them with a separate set of equations not carried across here
- `biosphere.allometric_equations.westfall_nsvb.coefficients`: `get_nsvb_model(species_code, division=..., stand_origin=...)` returns an `NsvbModel` pairing the wood and bark fit with the foliage fit for one species, and `compute_agb_tonnes` sums them. Both fits are looked up independently, taking the most specific coefficient set the paper publishes for the species - species and ecodivision, then the species' ecodivision-pooled entry, then the species' Jenkins group - so a species NSVB never fitted still gets an equation, and `coefficient_level` records which level was used. `published_divisions` lists the ecodivisions a species has a distinct fit for
- `biosphere.allometric_equations.westfall_nsvb.models`: `DEFAULT_STAND_ORIGIN` is planted, which is what `get_nsvb_model` and `create_nsvb_model_generator` assume when the caller gives no `stand_origin`. Only slash pine and loblolly pine are fitted separately by stand origin, since a 30 cm, 20 m loblolly is 2.7% heavier planted than natural through the foliage form - so `StandOrigin.NATURAL` has to be passed explicitly for naturally regenerated stands
- `biosphere.allometric_equations.westfall_nsvb.models`: `compute_agb_tonnes_with_error` draws the allometric residual, and `agb_residual_sd_tonnes` exposes it. NSVB fits with `1/D²` weights, which makes the published `SIGMA` a residual standard deviation *per inch of diameter* rather than an absolute one, so the spread is taken as `SIGMA * D` and scales with tree size the way the fitting residuals do. The statistic is the one NSVB publishes for total biomass (`TT_DW_ADJ`, tables S12 and S13) rather than the two components combined in quadrature, which would assume their residuals are independent when both are driven by the same tree. A species with its own fit statistic uses it and everything else falls back to the national one, including the few species fitted on a single felled tree, for which NSVB publishes an infinite `SIGMA`
- `biosphere.allometric_equations.westfall_nsvb.coefficients`: `create_nsvb_model_generator` - resamples among the alternative coefficient sets NSVB publishes for a species across ecodivisions, giving a bootstrap a way to carry allometric parameter uncertainty and not only residual error. Validated against a case-resampling refit of the raw felled-tree data behind the fits (Radtke et al. 2023): the ecodivision spread runs about 2.6x the true parameter sampling spread for wood and bark and about 3.0x for foliage, because regional allometric differences are real signal rather than sampling noise. That is deliberately the conservative direction, and it is second-order regardless - on a 30 cm, 20 m shortleaf pine the generator moves AGB by 1.4% against 12.1% from residual error. A species with only a pooled fit has nothing to resample and yields a constant model

## [0.48.12](https://github.com/isometric/calculation-library/releases/tag/v0.48.12)

Dependency version update.

## [0.48.11](https://github.com/isometric/calculation-library/releases/tag/v0.48.11)
...
## [0.48.8](https://github.com/isometric/calculation-library/releases/tag/v0.48.8)
